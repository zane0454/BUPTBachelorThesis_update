...
本仓库为按照《北京邮电大学2026届本科毕业设计（论文）指导手册》制作的毕业设计及其文档的Latex模板，但不保证完全符合指导手册的要求和同学校提供的模板完全一致。

> 本README并非LaTeX的使用教程,**使用教程请参考guidence.md**，默认您对于LaTeX有着基础的了解并懂得如何使用搜索引擎或者大模型来进行微调
...
2. 我只会用LaTeX。
3. ~~Overleaf的多人协作功能非常好用~~，如果不会导入字体请跳过这条，在本地编译^^
4. **无法接受typst的bug**

### 封面页和诚信申明页从哪里来的？

从学校提供的官方Word模板中编辑好导出为PDF文件，因为这两个文件使用Latex排版非常复杂，排版的效果也不好，2026-12-1更新封面页，但诚信申明页不用电子编辑，且极为难排版
...
提供一个统计字数的方法，将下面代码写到一个新的.tex文件中（注意统计的文件目录），然后使用**pdflatex引擎**编译该文件就可以得到统计字数了需要排版跨页的表格使用什么环境？

```tex
% reference: https://blog.csdn.net/weixin_43287372/article/details/144809833
\documentclass{article}
\usepackage{verbatim}

% 启用 shell-escape（Overleaf 默认已启用，无需额外设置）
\begin{document}

% 统计所有以 chapters 目录下 chapter 开头的 .tex 文件（例如: chapter1.tex, chapter2.tex）
\immediate\write18{texcount -inc -sum -merge chapters/chapter*.tex > wordcount.txt}
...
`doc/trans.pdf`是外文资料的路径

```tex
\blankmatter
\includepdf[pages=1, scale=0.9, pagecommand={%
    \begin{tikzpicture}[remember picture, overlay]
        \node[anchor=north, yshift=-2cm] at (current page.north) {\heiti{\zihao{3}{\textbf{外文资料}}}};
    \end{tikzpicture}
}]{docs/qos.pdf}

\includepdf[pages=2-]{docs/trans.pdf}
```
